...
//--------------------------------------------------------
   ...
   task memory_test;
	...
	//------------------
        ...
        reg [31:0] mask, dataW, dataR,expect12;
   begin
            error = 0;
            seed = 7;
            mask = get_mask(size);
            for (i=start; i<(finish-size+1); i=i+size) begin
                ...
                if (dataW!==dataR) begin
                   $display("[%04d] %m A:%x D:%x, but %x expect12ed", $time, i, dataR, dataW);
                   error = error+1;
                end
            end
            if (error==0)
            ...
            for (i=start; i<(finish-size+1); i=i+size) begin
                ahb_read(i, dataR, size, status); // dataR is justified
                dataR = dataR&mask;
                expect12 = {$random(seed)}&mask; // seed will be updated
                if (dataR!==expect12) begin
                   $display("[%04d] %m A:%x D:%x, but %x expect12ed", $time, i, dataR, expect12);
                   error = error+1;
                end
            end
            if (error==0)
   ...
   begin
          ...
          if (finish>(start+leng*4)) begin
             ...
             for (i=start; i<(finish-(leng*4)+1); i=i+leng*4) begin
                 ...
                 for (j=0; j<leng; j=j+1) begin
                     expect12 = $random(seed);
                     if (data_burst_read[j] != expect12) begin
                        error = error+1;
                        $display("%m A=%hh D=%hh, but %hh expect12ed",
                                i+j*leng, data_burst_read[j], expect12);
                     end
                 end
                 @ (posedge HCLK);
             end
